...
import { StoneState, SimulationEvent, MoveRecord, SimulationInput, CandidateEvent } from "./types.js";
import * as SimMath from "./math.js";

/**
 * Симулирует полный ход и возвращает запись всех событий
 */
export function simulateMove(input: SimulationInput, moveId: number): MoveRecord {
    const events: SimulationEvent[] = [];
    let currentTime = 0;
    let stones = input.stones.map(s => ({ ...s }));
    ...
    while (true) {
        ...
        const validCandidates = candidates.filter(c => {
            if (c.eventType === 'STOP') return true;
            return c.time > 0.1;
        });
        ...
        // Обновляем флаги
        if (nextEvent.eventType === 'CLEAN_PASS') hasPassedThrough = true;
        if (nextEvent.eventType === 'GOAL') isGoalScored = true;
        if (nextEvent.eventType === 'COLLISION') hitObstacle = true;
...
function applyEvent(
    ...
    flags: { hasPassedThrough: boolean; isGoalScored: boolean; hitObstacle: boolean }
): void {
...
function determineResult(
    ...
    hitObstacle: boolean,
    playerIndex: number
): { type: 'GOAL' | 'FOUL' | 'OUT' | 'CLEAN_PASS' | 'MISS'; nextPlayer: number } {
