...
import { 
    GameState, stones, aiThinkingTime, accuracyEnabled, spreadFactor,
    MAX_FORCE
} from "./state.js";
...
export function applyAIMove(move: AIMove): void {
    ...
    // === ИСПРАВЛЕНО: Применяем разброс (если точность выключена) ===
    const spreadValue = accuracyEnabled ? 0 : spreadFactor;
    const spread = (move.force / MAX_FORCE) ** 2 * spreadValue;
    const finalAngle = GameMath.randomGaussian(baseAngle, spread);
    
    // ОТЛАДКА
    const angleDeviationDeg = (finalAngle - baseAngle) * 180 / Math.PI;
    console.log(`[AI] Удар: сила=${move.force.toFixed(1)}, угол=${(baseAngle * 180 / Math.PI).toFixed(1)}°, разброс=${(spread * 180 / Math.PI).toFixed(2)}°, отклонение=${angleDeviationDeg.toFixed(2)}°, точность=${accuracyEnabled}`);
