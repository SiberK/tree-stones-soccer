...
import {
    ...
    accuracyEnabled, spreadFactor, STOP_THRESHOLD_RATIO,
    angleSteps, forceSteps
} from "../state.js";
import { forceForDistance } from "../simulation/kinematics.js";
import {
...
function buildCandidatesForStriker(
	...
	// было: const forceMin = (dGate + 2 * R) * K + thr;
	...
	for (let i = 0; i < angleSteps; i++) {
		...
		const forceMin = forceForDistance(dGate + 2 * R, R, FRICTION, STOP_THRESHOLD_RATIO);
        ...
        let forceMax = forceForDistance(dMaxStop, R, FRICTION, STOP_THRESHOLD_RATIO);
