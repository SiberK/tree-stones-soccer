...
export class Stone {
    ...
    generateSprite() {
        ...
        if (stoneImg.complete && stoneImg.naturalWidth > 0) {
            const texSize = 200;
            spriteCtx.drawImage(stoneImg, this.textureSection.sx % (stoneImg.naturalWidth - texSize), this.textureSection.sy % (stoneImg.naturalHeight - texSize), texSize, texSize, centerX - this.radius, centerY - this.radius, this.radius * 2, this.radius * 2);
        }
        else {
        ...
        // 3. ОБЪЁМ И СВЕТОТЕНЬ
        const volGrad = spriteCtx.createRadialGradient(centerX - this.radius * 0.3, centerY - this.radius * 0.3, this.radius * 0.1, centerX, centerY, this.radius);
    ...
    draw(ctx) {
        ...
        ctx.drawImage(this.sprite, this.x - size / 2, this.y - size / 2);
    }
}
