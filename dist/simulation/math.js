 ...
 */
export function velocityAtTime(v0, t) {
    return v0 * Math.exp(-K * t);
}
/**
 * Вычисляет позицию камня в момент времени t
 */
export function positionAtTime(x0, v0, t) {
 ...
 */
export function stopTime(v0x, v0y) {
 ...
 */
export function findGateIntersection(startPos, dir, // единичный вектор направления
gate1, gate2) {
 ...
 */
export function calculateCollisionTime(s1, s2) {
    if (s1.isOut || s2.isOut)
        return null;
    ...
    const c = dx * dx + dy * dy - Math.pow((s1.radius + s2.radius), 2);
    ...
    if (discriminant < 0)
        return null; // Столкновения не будет
 ...
 */
export function calculateOutTime(stone) {
    if (stone.isOut)
        return null;
    ...
    if (minTime === Infinity || minTime < 0.1)
        return null;
 ...
 */
function solveForBoundary(x0, v0, target) {
    ...
    if (arg <= 0)
        return null; // Не достигнет границы
 ...
 */
function timeToReachDistance(distance, speed) {
    if (speed < 0.001)
        return null;
 ...
 */
export function checkCleanPass(striker, gate1, gate2) {
    if (striker.isOut || gate1.isOut || gate2.isOut)
        return null;
    ...
    const intersection = findGateIntersection(startPos, dir, { x: gate1.x, y: gate1.y }, { x: gate2.x, y: gate2.y });
    if (!intersection)
        return null;
    ...
    const distanceToA = Math.hypot(intersection.point.x - gate1.x, intersection.point.y - gate1.y);
    const distanceToB = Math.hypot(intersection.point.x - gate2.x, intersection.point.y - gate2.y);
 ...
 */
export function checkGoal(striker, allStones, skipGateCheck = false) {
    if (striker.isOut)
        return null;
    const speed = Math.hypot(striker.vx, striker.vy);
    if (speed < 0.001)
    ...
    if (!skipGateCheck) {
        ...
        if (others.length === 2) {
            ...
            if (hasGate) {
                const gateIntersection = findGateIntersection(startPos, dir, { x: others[0].x, y: others[0].y }, { x: others[1].x, y: others[1].y });
